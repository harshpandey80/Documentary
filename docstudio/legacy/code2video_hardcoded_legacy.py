...
class Code2VideoEngine:
    ...
    def generate_tactical_map_video(
        ...
        """
        frames_dir = self.cache_dir / f"map_frames_{abs(hash(topic))}"
        ...
        for i in range(total_frames):
            ...
            if is_vertical:
                ...
                draw.text((65, 100), f"TACTICAL BATHYMETRY // {topic.upper()[:32]}", fill=(240, 245, 255))
                draw.text((65, 135), f"LOCATION: CHALLENGER DEEP // LAT 11°22.4'N  LON 142°35.5'E", fill=(80, 200, 240))
                draw.text((65, 168), f"TELEMETRY: DEPTH 10,994M (36,070 FT) // PRESSURE: 1,086 BAR", fill=(160, 180, 205))
                draw.text((65, 200), f"ACOUSTIC FREQUENCY: 14.8 KHZ // PHASE: {int(progress * 4) + 1}", fill=(245, 180, 40))
                ...
                draw.text((blip_pos[0]+12, blip_pos[1]-8), "ANOMALY CONTACT", fill=(255, 100, 100))

                # Bathymetric Trench Cross-Section (Mid-Lower Section)
                ...
                draw.text((width // 2 - 110, trench_bottom_y + 8), "CHALLENGER DEEP -10,994M", fill=(100, 220, 255))

                # Oscilloscope / Seismic Frequency Monitor (Bottom)
                osc_top = int(height * 0.84)
                osc_bottom = int(height * 0.94)
                draw.rectangle([40, osc_top, width - 40, osc_bottom], fill=(12, 18, 26), outline=(40, 80, 110), width=1)
                draw.text((55, osc_top + 10), "HYDROPHONE FREQUENCY OSCILLOGRAM (14.8 KHZ PEAK)", fill=(130, 160, 190))
                ...
                draw.text((105, 115), f"PHASE {int(progress * 4) + 1} // LAT 11°22.4'N  LON 142°35.5'E // DEPTH: -10,994M", fill=(130, 180, 220))
    ...
    def generate_dossier_redaction_video(
        ...
        """
        frames_dir = self.cache_dir / f"dossier_frames_{abs(hash(topic))}"
        frames_dir.mkdir(parents=True, exist_ok=True)
        ...
        doc_bottom = int(height * 0.94) if is_vertical else int(height * 0.92)
        ...
        for i in range(total_frames):
            ...
            draw.text((inner_left, doc_top + 105), "DEPARTMENT OF THE NAVY // OFFICE OF NAVAL INTELLIGENCE", fill=(60, 52, 44))
            draw.text((inner_left, doc_top + 135), f"SUBJECT: {topic.upper()[:44]} - TELEMETRY DOSSIER", fill=(85, 75, 65))
            draw.text((inner_left, doc_top + 165), f"FILE ID: ONI-ARC-79401 // LAT: 11°22'N  LON: 142°35'E", fill=(110, 100, 90))
            ...
            if progress > 0.35:
                ...
                draw.text((stamp_x1 + 30, stamp_y1 + 18), "DECLASSIFIED // BY EXECUTIVE ORDER", fill=(190, 30, 30))
                draw.text((stamp_x1 + 45, stamp_y1 + 54), "NATIONAL ARCHIVES & RECORDS SERVICE", fill=(160, 45, 45))
